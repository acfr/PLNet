...
class OneCycleLinearSchedule(
  ...
  def __init__(self,
    ...
    self.lr_max = lr_max
    self.lr_min = lr_max/ratio 
    self.peak = peak
    self.decay = decay
    
  @tf.function
  def __call__(self, step):
    x = tf.cast(step, tf.float32) / self.tot_steps
    p, d = self.peak, self.decay
    if x <= p:
      lr = self.lr_max * x / p
    elif x <= d:
      lr = self.lr_max * (d - x) / (d - p) + self.lr_min * (x - p) / (d - p)
    else:
      lr = self.lr_min * (1.0 - x) / (1.0 - d)
    return lr
