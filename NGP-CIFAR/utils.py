...
flags.DEFINE_string('cifar100_c_path', "CIFAR-100-C",
...
flags.DEFINE_bool('download_data', True,
...
flags.DEFINE_integer('per_core_batch_size', 256, 
...
def load_cifar100_c(corruption_name,
  ...
  if use_bfloat16:
    ...
    dtype = tf.float32
  data_dir = os.path.join(os.path.expanduser('~'), 'tensorflow_datasets', path)
  image_file = os.path.join(data_dir, '{0}.npy'.format(corruption_name))
  label_file = os.path.join(data_dir, 'labels.npy')
  image = np.load(image_file)[10000*(corruption_intensity-1):10000*(corruption_intensity), ...]
  label = np.load(label_file)[10000*(corruption_intensity-1):10000*(corruption_intensity), ...]
  ...
  label = tf.convert_to_tensor(label, dtype=dtype)
  ...
  # dataset = tf.data.NumpyIterator(data)
...
def aggregate_corrupt_metrics(metrics,
  ...
  results = {
      '{}_nll_mean_corrupted'.format(prefix): 0.,
      '{}_kl_mean_corrupted'.format(prefix): 0.,
      '{}_elbo_mean_corrupted'.format(prefix): 0.,
      '{}_accuracy_mean_corrupted'.format(prefix): 0.,
      '{}_ece_mean_corrupted'.format(prefix): 0.,
      '{}_member_acc_mean_corrupted'.format(prefix): 0.,
      '{}_member_ece_mean_corrupted'.format(prefix): 0.
  }
  ...
  for intensity in range(1, max_intensity + 1):
    ...
    for i in range(len(corruption_types)):
      ...
      nll[i] = metrics['{0}_nll_{1}'.format(prefix, dataset_name)].result()
      if '{0}_kl_{1}'.format(prefix, dataset_name) in metrics.keys():
        kl[i] = metrics['{0}_kl_{1}'.format(prefix, dataset_name)].result()
      else:
        kl[i] = 0.
      if '{0}_elbo_{1}'.format(prefix, dataset_name) in metrics.keys():
        elbo[i] = metrics['{0}_elbo_{1}'.format(prefix, dataset_name)].result()
      else:
        elbo[i] = 0.
      acc[i] = metrics['{0}_accuracy_{1}'.format(prefix, dataset_name)].result()
      ece[i] = (metrics['{0}_ece_{1}'.format(prefix, dataset_name)].result())['ece']
      if '{0}_member_acc_mean_{1}'.format(prefix,
                                          dataset_name) in metrics.keys():
        member_acc[i] = metrics['{0}_member_acc_mean_{1}'.format(
            prefix, dataset_name)].result()
      else:
        member_acc[i] = 0.
      if '{0}_member_ece_mean_{1}'.format(prefix,
                                          dataset_name) in metrics.keys():
        member_ece[i] = metrics['{0}_member_ece_mean_{1}'.format(
            prefix, dataset_name)].result()
        member_ece[i] = 0.
      if corrupt_diversity is not None:
        disagreement[i] = (
      ...
      if log_fine_metrics or output_dir is not None:
        fine_metrics_results['{0}_nll_{1}'.format(prefix,
                                                  dataset_name)] = nll[i]
        fine_metrics_results['{0}_kl_{1}'.format(prefix,
                                                 dataset_name)] = kl[i]
        fine_metrics_results['{0}_elbo_{1}'.format(prefix,
                                                   dataset_name)] = elbo[i]
        fine_metrics_results['{0}_accuracy_{1}'.format(prefix,
                                                       dataset_name)] = acc[i]
        fine_metrics_results['{0}_ece_{1}'.format(prefix,
                                                  dataset_name)] = ece[i]
        if corrupt_diversity is not None:
    ...
    results['{0}_nll_mean_{1}'.format(prefix, intensity)] = avg_nll
    results['{0}_kl_mean_{1}'.format(prefix, intensity)] = avg_kl
    results['{0}_elbo_mean_{1}'.format(prefix, intensity)] = avg_elbo
    results['{0}_accuracy_mean_{1}'.format(prefix, intensity)] = avg_accuracy
    results['{0}_ece_mean_{1}'.format(prefix, intensity)] = avg_ece
    results['{0}_nll_median_{1}'.format(prefix, intensity)] = np.median(nll)
    results['{0}_kl_median_{1}'.format(prefix, intensity)] = np.median(kl)
    results['{0}_elbo_median_{1}'.format(prefix, intensity)] = np.median(elbo)
    results['{0}_accuracy_median_{1}'.format(prefix,
                                             intensity)] = np.median(acc)
    results['{0}_ece_median_{1}'.format(prefix, intensity)] = np.median(ece)
    results['{0}_member_acc_mean_{1}'.format(prefix,
                                             intensity)] = avg_member_acc
    results['{0}_member_ece_mean_{1}'.format(prefix,
                                             intensity)] = avg_member_ece
    results['{}_nll_mean_corrupted'.format(prefix)] += avg_nll
    results['{}_kl_mean_corrupted'.format(prefix)] += avg_kl
    results['{}_elbo_mean_corrupted'.format(prefix)] += avg_elbo
    results['{}_accuracy_mean_corrupted'.format(prefix)] += avg_accuracy
    results['{}_ece_mean_corrupted'.format(prefix)] += avg_ece
    results['{}_member_acc_mean_corrupted'.format(prefix)] += avg_member_acc
    results['{}_member_ece_mean_corrupted'.format(prefix)] += avg_member_ece
    if corrupt_diversity is not None:
  ...
  results['{}_nll_mean_corrupted'.format(prefix)] /= max_intensity
  results['{}_kl_mean_corrupted'.format(prefix)] /= max_intensity
  results['{}_elbo_mean_corrupted'.format(prefix)] /= max_intensity
  results['{}_accuracy_mean_corrupted'.format(prefix)] /= max_intensity
  results['{}_ece_mean_corrupted'.format(prefix)] /= max_intensity
  results['{}_member_acc_mean_corrupted'.format(prefix)] /= max_intensity
  results['{}_member_ece_mean_corrupted'.format(prefix)] /= max_intensity
  if corrupt_diversity is not None:
    for key in diversity_keys:
