...
def basic_block(inputs,
  ...
  Conv2D_1 = make_conv2d_layer(use_spec_norm,  # pylint: disable=invalid-name
                             spec_norm_iteration,
                             1.0,
                             use_bias=True)
...
def wide_resnet_sngp(input_shape, batch_size, depth, width_multiplier,
  ...
  #                            spec_norm_iteration,
  #                            spec_norm_bound_0)
  Conv2D_0 = make_conv2d_layer(use_spec_norm,  # pylint: disable=invalid-name
                             spec_norm_iteration,
                             spec_norm_bound_0)
...
if __name__ == "__main__":
    ...
    model = wide_resnet_sngp(input_shape, batch_size, depth, width_multiplier,
                     num_classes, l2, use_mc_dropout, use_filterwise_dropout,
                     dropout_rate, use_gp_layer, gp_input_dim, gp_hidden_dim,
                     gp_scale, gp_bias, gp_input_normalization,
                     gp_random_feature_type, gp_cov_discount_factor,
    ...
    model.build((batch_size, image_size, image_size, cin))
    print('Model number of weights: %s', model.count_params())
    y = model(tf.random.normal((batch_size, image_size, image_size, cin)))
