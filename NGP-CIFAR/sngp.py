...
from schedules import WarmUpPiecewiseConstantSchedule
...
flags.DEFINE_float('spec_norm_bound', 0.99,
                   'Upper bound to spectral norm of weight matrices.')
flags.DEFINE_bool('use_batch_norm', True,
                  'Whether to use batch normalization.')
flags.DEFINE_integer('width_multiplier', 10,
                  'width multiplier for WideResNet.')
...
def main(argv):
  ...
  if FLAGS.use_batch_norm:
    FLAGS.output_dir = f'{FLAGS.output_dir}/{FLAGS.dataset}/seed{FLAGS.seed}-sngp-w{FLAGS.width_multiplier}-sn{FLAGS.spec_norm_bound:.2f}-bn'
  else:
    FLAGS.output_dir = f'{FLAGS.output_dir}/{FLAGS.dataset}/seed{FLAGS.seed}-sngp-w{FLAGS.width_multiplier}-sn{FLAGS.spec_norm_bound:.2f}'
  ...
  if FLAGS.use_spec_norm:
    logging.info('Input layer: Use Spectral Normalization with norm bound %.2f',
                  FLAGS.spec_norm_bound_0)
    logging.info('BiLip layer: Use Spectral Normalization with norm bound %.2f',
  ...
  model = wide_resnet_sngp(
      ...
      spec_norm_bound_0=FLAGS.spec_norm_bound_0,
  ...
  metrics = {
      ...
      'train/l2_loss': tf.keras.metrics.Mean(),
  ...
  def train_step(iterator, step):
    ...
    def step_fn(inputs, step):
      ...
      metrics['train/l2_loss'].update_state(l2_loss)
  ...
  for epoch in range(initial_epoch, FLAGS.train_epochs):
    ...
    for dataset_name, test_dataset in datasets_to_evaluate.items():
      ...
      logging.info('Starting to run eval at epoch: %s', epoch+1)
    ...
    logging.info('Train Loss: %.4f, L2_loss: %.4f, Accuracy: %.2f%%',
                 metrics['train/loss'].result(),
                 metrics['train/l2_loss'].result(),
