...
import datasets
# from ftn_gp import cftn_gp
from cftn import cftn_gp
from schedules import WarmUpPiecewiseConstantSchedule, OneCycleLinearSchedule
...
def main(argv):
  del argv  # unused arg
  FLAGS.output_dir = f'{FLAGS.output_dir}/{FLAGS.dataset}/seed{FLAGS.seed}-fngp{FLAGS.spec_norm_bound:.2f}'
  ...
  FLAGS.mu = (1. - FLAGS.spec_norm_bound) ** 4
  FLAGS.nu = (1. + FLAGS.spec_norm_bound) ** 4 
  logging.info('Lipschitz bounds: mu=%.4f, nu=%.3f',
                  FLAGS.mu, FLAGS.nu)
  model = cftn_gp(
      input_shape=(32, 32, 3),
...
#   logging.info('Model output shape: %s', model.output_shape)
  ...
  # lr_schedule = OneCycleLinearSchedule(
  #   lr_max=FLAGS.base_learning_rate,
  #   tot_steps=steps_per_epoch*FLAGS.train_epochs,
  #   ratio=20.,
  #   peak=0.25,
  #   decay=0.75)
  ...
  metrics = {
      ...
      'train/l2loss': tf.keras.metrics.Mean(),
  ...
  def train_step(iterator, step):
    ...
    def step_fn(inputs, step):
      ...
      if tf.equal(step, 0) and FLAGS.gp_cov_discount_factor < 0:
        ...
        model.gp.reset_covariance_matrix()
      ...
      with tf.GradientTape() as tape:
        ...
        l2_loss = model.l2_loss() #sum(model.losses)
      ...
      metrics['train/l2loss'].update_state(l2_loss)
